...
@interface LJInstrumentView () <CAAnimationDelegate>
/**
 *  圆盘开始角度
 */
...
//    layer.position = CGPointMake(self.ZK_width * 0.5, self.ZK_width * 0.5);
//    layer.bounds = CGRectMake(0, 0, 2, _scaleRadius - 10);
//    layer.cornerRadius = 4;
//    layer.transform = CATransform3DMakeRotation(-startAngle, 0, 0, 1);
...
-(void)DrawScaleValueWithDivide:(NSInteger)divide{
    ...
    for (NSUInteger i = 0; i <= divide; i++) {
        ...
        NSString *tickText = [NSString stringWithFormat:@"%ld",(divide - i)*200/divide];
        //默认label的大小23 * 14
        UILabel *text = [[UILabel alloc] initWithFrame:CGRectMake(point.x - 5, point.y - 7, 30, 14)];
        text.text = tickText;
        text.font = [UIFont systemFontOfSize:10.f];
        text.textColor = [UIColor colorWithHexString:@"000000"];
...
- (CGPoint)calculateTextPositonWithArcCenter:(CGPoint)center
                                       Angle:(CGFloat)angel
{
    CGFloat x = (self.scaleValueRadius + 3.5 * self.lineWidth)* cosf(angel);
    CGFloat y = (self.scaleValueRadius + 3.5 * self.lineWidth)* sinf(angel);
...
- (void)runSpeedProgress{
    [self.speedLabel countFrom:0 to:_speedValue withDuration:1.0];
    [UIView animateWithDuration:0.5 animations:^{
        self.progressLayer.strokeEnd = 0.01 * self.speedValue/2;

//        CGFloat angel = self.endAngle * 180 * M_PI/2 ;
//        CGFloat angel = self.speedValue/2/200 * self.endAngle * 180 * M_PI/2 ;
//        CAKeyframeAnimation *anim = [CAKeyframeAnimation animationWithKeyPath:@"position"];
//        NSValue *p1 = [NSValue valueWithCGPoint:self.center];
//        NSValue *p2 = [NSValue valueWithCGPoint:self.center];
//        NSValue *p3 = [NSValue valueWithCGPoint:self.center];
...
//        self.arrowLayer.transform = CATransform3DMakeRotation(angel + 0.2, 0, 0, 1);
    }];
...
-(UILabel *)speedLabel{
    if (!_speedLabel) {
        ...
        _speedLabel.frame = CGRectMake(0, 0, 50, 30);
        _speedLabel.adjustsFontSizeToFitWidth = YES;
        _speedLabel.method = UILabelCountingMethodLinear;
        _speedLabel.format = @"%.1f";
        _speedLabel.font = [UIFont systemFontOfSize:30.0f];
